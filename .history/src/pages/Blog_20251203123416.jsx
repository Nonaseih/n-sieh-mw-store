...
// Add this comment at the very top if you're using JSDoc/ESLint override
...
export default function Blog() {
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-[#faf7f0] pt-32 pb-20">
            ...
            >
              <div className="relative">
                <img
                  ...
                  className="w-full h-96 md:h-[600px] object-cover group-hover:scale-105 transition duration-700"
                />
        ...
        <div className="bg-[#f5f0e6] py-20 mt-32">
          <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-12 text-center">
            {[
              { icon: ShoppingBag, title: "Free Express Shipping", desc: "Worldwide delivery within 3–5 days" },
              { icon: Headphones, title: "Personal Stylist Support", desc: "Expert guidance, anytime" },
              { icon: CreditCard, title: "Secure & Discreet Payment", desc: "100% safe transactions" }
            ].map((item, i) => (
              <div key={i} className="text-amber-900">
                <item.icon size={56} className="mx-auto mb-4 text-amber-800" />
                <h3 className="text-2xl font-bold">{item.title}</h3>
                <p className="mt-3 text-amber-700 text-sm">{item.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        {showBlog && (
          <>
            ...
            >
              <button
                onClick={() => setShowBlog(false)}
                className="fixed top-8 right-8 z-50 bg-white rounded-full p-4 shadow-2xl hover:scale-110 transition-all"
              >
                <X size={36} className="text-amber-900" />
              </button>

              <div className="pt-40 pb-24 text-center">
                <h1 className="text-8xl md:text-9xl font-black text-amber-900">JOURNAL</h1>
              ...
              <div className="max-w-7xl mx-auto px-6 pb-32 grid md:grid-cols-3 gap-12">
                {[1, 2, 3, 4, 5, 6].map((i) => (
                  ...
                  >
                    <img
                      src={`https://images.unsplash.com/photo-15${i}05774583-5a5e2f0c5d2e?w=800&h=1000&fit=crop&auto=format&q=80`}
                      alt={`Fashion blog post ${i}`}
                      className="w-full h-80 object-cover"
                    />
                    <div className="p-8">
                      <h3 className="text-2xl font-bold text-amber-900">
                        {i === 1 ? "How to Style Silk This Season" : `The Art of Layering ${i}`}
                      </h3>
                      <p className="mt-4 text-amber-700">Elevate your wardrobe with effortless luxury and timeless pieces...</p>
      ...
      {/* Trigger Button - You probably want one! */}
      <button
        onClick={() => setShowBlog(true)}
        className="fixed bottom-10 right-10 bg-amber-900 text-white px-8 py-4 rounded-full text-lg font-bold shadow-2xl hover:bg-amber-800 transition z-30"
      >
        Open Journal
      </button>
    </>
